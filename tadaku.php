...
function curl($url,$use=false){
	$socks = explode("\n", 
	file_get_contents("arifsocks.txt"));
	$i=0;
...
while($a<true){
	...
	$amount = @json_decode($c[1],true)['body']['amount'];
		...
		echo "\e[33mChange Socks!\e[0m\n";
		$cookie = curl("https://e.gift.id/u/83lw5mzhy1v46",true);
    	}
	echo "\e[91mChecking\e[0m ";
	$o+=1;
	if($o>2) $o=0;
	echo "[{$arr[$o]}] {$c[0]}\r";
}
